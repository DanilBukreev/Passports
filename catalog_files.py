...
# sg.theme_previewer()
layout2 = [
    [sg.Text('Укажите путь до Шаблонов Паспортов:', size=(35, 1)), sg.InputText(size=(80, 1)), sg.FolderBrowse()],
...
window.close()
    ...
    #       for i in range(0, len(files)):
     #           splt_files = list(files[i])
     #           print(splt_files)
     #           ful_number = ''.join([splt_files[0], splt_files[1], splt_files[2]])
     #           wbSearch = Workbook()
     #           wbSearch = load_workbook(Excel_path)
     #           wsSearch = wbSearch.worksheets[0]
     #           for cell in wsSearch["A"]:
